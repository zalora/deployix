deployix: secrets-dirs: let
  ...
  secret-files = dir: builtins.filter (x:
    x != null
  ) (deployix.lib.map-attrs-to-list (name: type:
  ...
  inherit (deployix.lib.nix-exec) sequence unit bind;

  inherit (deployix.nix-exec) decrypt-file getenv getpass;
  ...
  io-decrypted = bind io-pass (pass: sequence (builtins.concatLists (map (dir:
    map (secret-file:
      deployix.lib.nix-exec.map (value:
...
in decrypt: deployix.lib.nix-exec.map builtins.listToAttrs (if decrypt
  then io-decrypted
  else io-cleartext
)
